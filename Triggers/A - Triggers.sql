...
--For this problem, we only need  the trigger to work on the UPDATE statement,as inserting does not change and old balance and deleting does not generate a new balance.
...
AS
	...
	BEGIN
        ...
        WHERE D.BalanceOwing <> I.BalanceOwing -- the reason for this line here is that our UPDATE statement might affect multiple rows, and not all of the rows
        --will have a change in th BalanceOwing.
	    IF @@ERROR <> 0 
	    BEGIN
