...
--@@IDENTITY  is a global variable that holds the last-generated
-- IDENTITY value for a table with an IDENTITY column
-- (PK).
    ...
    BEGIN -- {
        ...
        -- The EXISTS() function checks to see if there are any rows resulting from a query
            ...
            BEGIN -- { -- This BEGIN/END is needed, because of two SQL statements
                ...
                END
                    ELSE --An Error value other than 0 means there is a problem
                    BEGIN 
...
EXEC LookupClubMembers 'NASA1' -- Although this  returns zero rows,it is a valid result for this SPROC
...
AS
    ...
    --Because I want to excecute more than 1 statement.
    BEGIN
